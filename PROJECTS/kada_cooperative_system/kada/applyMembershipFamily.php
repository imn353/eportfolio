...
$stmt = $con->prepare("SELECT * FROM tb_member WHERE mem_id = ? OR mem_ic = ? OR mem_pf = ? OR mem_email = ?");
$stmt->bind_param("isis", $fnostaff, $fic, $fnopf, $femail);
$stmt->execute();
$result = $stmt->get_result();
$row = $result->fetch_assoc();
if ($row) {
    if ($row['mem_id'] == $fnostaff) {
        echo "<script>
                alert('ID Staf ini telah wujud dalam pangkalan data.');
                window.location.replace('applyMembership.php');
            </script>";
    } elseif ($row['mem_ic'] == $fic) {
        echo "<script>
                alert('No. Kad Pengenalan ini telah wujud dalam pangkalan data.');
                window.location.replace('applyMembership.php');
            </script>";
    } elseif ($row['mem_pf'] == $fnopf) {
        echo "<script>
                alert('No. PF ini telah wujud dalam pangkalan data.');
                window.location.replace('applyMembership.php');
            </script>";
    } elseif ($row['mem_email'] == $femail) {
        echo "<script>
                alert('Email ini telah wujud dalam pangkalan data.');
                window.location.replace('applyMembership.php');
            </script>";
    }
}
...
<div class="container py-5"> <br><br><br>
    <div class="card shadow">
        ...
        <div class="card-body p-4">
            ...
            <form method="POST" action="applyMembershipCharity.php" class="needs-validation" novalidate>
