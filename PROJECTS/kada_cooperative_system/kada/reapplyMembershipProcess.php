...
$memberData = [
    ...
    'fpass' => $_POST['fpass'], // Will be hashed later
...
$query = "SELECT m_appstatus FROM tb_membership WHERE m_nostaff = '{$memberData['fnostaff']}' AND m_quit_date IS NULL ORDER BY m_appdate DESC;";
...
if (mysqli_query($con, $sql1)) {
    ...
    $sql2 = "UPDATE tb_member SET 
        mem_ic = '{$memberData['fic']}', 
        mem_pf = '{$memberData['fnopf']}', 
        mem_name = '{$memberData['fname']}', 
        mem_birthdate = '$fbirthdate',
        mem_status = '{$memberData['fstatus']}', 
        mem_address = '{$memberData['fhaddr']}', 
        mem_postcode = '{$memberData['fhpostcode']}', 
        mem_state = '{$memberData['fhstate']}', 
        mem_gender = '{$memberData['fgender']}', 
        mem_religion = '{$memberData['freligion']}', 
        mem_race = '{$memberData['frace']}', 
        mem_email = '{$memberData['femail']}', 
        mem_password = '$hashedPassword', 
        mem_position = '{$memberData['fposi']}', 
        mem_position_grade = '{$memberData['fgradeposi']}', 
        mem_office_address = '{$memberData['foaddr']}', 
        mem_office_postcode = '{$memberData['fopostcode']}', 
        mem_office_state = '{$memberData['fostate']}', 
        mem_fax = '{$memberData['fnofax']}', 
        mem_tel = '{$memberData['fnotel']}', 
        mem_tel_house = '{$memberData['fnohouse']}', 
        mem_salary = '{$memberData['fsalary']}', 
        mem_bank = '{$memberData['fbank']}', 
        mem_bank_no = '{$memberData['fnoacc']}', 
        mem_fee_modal_share = '{$memberData['share']}', 
        mem_fee_modal_fee = '{$memberData['modalfee']}', 
        mem_fee_fixed_saving = '{$memberData['saving']}', 
        mem_fee_charity_fund = '{$memberData['charity']}', 
        mem_fee_saving = '{$memberData['depo']}',
        mem_application_id = '$app_id' 
    WHERE mem_id = '{$memberData['fnostaff']}'";

    if (mysqli_query($con, $sql2)) {
        // Insert family members
        $familyInsertSuccess = true;
        if (!empty($familyMembers)) {
            $sqldelete = "DELETE FROM tb_family WHERE f_member_id = '{$memberData['fnostaff']}'";
